...
static llvm::cl::OptionCategory clOptionsCategory("outer-loop-vectorize options");
...
static NestedPattern makePattern(const llvm::DenseSet<Operation *> &parallelLoops) {
  using matcher::For;
  return For(isVectorizableLoopPtrFactory(parallelLoops, -1));
}
...
struct OuterLoopVec : public FunctionPass<OuterLoopVec> {
  OuterLoopVec();
  OuterLoopVec(ArrayRef<int64_t> virtualVectorSize);
...
OuterLoopVec::OuterLoopVec()
    : vectorSizes(clVirtualVectorSize.begin(), clVirtualVectorSize.end()) {} 

OuterLoopVec::OuterLoopVec(ArrayRef<int64_t> virtualVectorSize) : OuterLoopVec() {
...
struct VectorizationStrategy {
  uint64_t minMaxStride = UINT64_MAX;
  Operation* loopToVectorize = nullptr;
...
static unsigned getStride(Value *iv, LoadOrStoreOp memoryOp) {
...
static unsigned calculateCost(Operation *loop, unsigned vecSize) {
  AffineForOp forOp = dyn_cast<AffineForOp>(loop);
  llvm::Optional<uint64_t> tripCount = getConstantTripCount(forOp);
  if(tripCount.hasValue() && tripCount.getValue() < vecSize) return UINT32_MAX;
  llvm::SmallVector<Operation *, 8> loadAndStoreOps;
  forOp.walk([&](Operation *op) {
    if (isa<AffineLoadOp>(op) || isa<AffineStoreOp>(op))
      loadAndStoreOps.push_back(op);
  });
  Value *iv = forOp.getInductionVar(); 
  unsigned maxStride = 0;
  for(auto memOp: loadAndStoreOps)
  {
    auto load = dyn_cast<AffineLoadOp>(memOp);
    auto store = dyn_cast<AffineStoreOp>(memOp);
    unsigned stride = load ? getStride(iv, load) : getStride(iv, store);
    stride *= forOp.getStep();
...
/// strategies.
// static LogicalResult analyzeProfitability(ArrayRef<NestedMatch> matches, unsigned vecSize, unsigned* maxStride) {
//   for (auto m : matches) {
//     if (failed(analyzeProfitability(m.getMatchedChildren(), vecSize, maxStride))) {
//       return failure();
//     }
//     if(m.getMatchedChildren().size() > 0) calculateCost(m.getMatchedOperation(), vecSize);
//   }
//   return success();
// }
...
isVectorizableLoopPtrFactory(const llvm::DenseSet<Operation *> &parallelLoops,
  ...
  return [&parallelLoops, fastestVaryingMemRefDimension](Operation &forOp) {
    ...
    auto vectorizableBody =
        isVectorizableLoopBodyForOuterLoopVec(loop, &memRefDim, vectorTransferPattern());
    if (!vectorizableBody)
      return false;
...
bool compareCosts(CostStruct* c1, CostStruct* c2) { return c1->maxStride < c2->maxStride; }


/// Applies vectorization to the current Function by searching over a bunch of
/// predetermined patterns.
void OuterLoopVec::runOnFunction() {
  FuncOp f = getFunction();

  // Thread-safe RAII local context, BumpPtrAllocator freed on exit.
  NestedPatternContext mlContext;
  llvm::DenseSet<Operation *> loops;
  llvm::DenseSet<Operation *> parallelLoops;
  f.walk([&loops](AffineForOp loop) {
      loops.insert(loop);
  });
  for(auto loop1 : loops)
  {
    bool isNotInnermostLoop = false;
    for(auto loop2 : loops)
    {
      if(loop1->isProperAncestor(loop2)) 
      {
        isNotInnermostLoop = true;
        break; 
      }
    }
    if(isNotInnermostLoop)
    {
      AffineForOp forOp = dyn_cast<AffineForOp>(loop1);
      if(isLoopParallel(forOp)) parallelLoops.insert(loop1);
    } 
  }
  // delete &loops;
  SmallVector<CostStruct*, 8> costs;
  auto pat = makePattern(parallelLoops); 
    LLVM_DEBUG(dbgs() << "\n******************************************");
    LLVM_DEBUG(dbgs() << "\n******************************************");
    ...
    for (unsigned i = 0; i < matches.size(); i++) {
      CostStruct *c = new CostStruct;
      NestedMatch* m = &matches[i];
      c->m = m;
      c->maxStride = calculateCost(m->getMatchedOperation(), vectorSizes[0]);
      // printf("&&&&&&&&&&&&&&&&&&&%u\n", c->maxStride);
      costs.push_back(c);
    }
  ...
  for(auto c : costs)
  {
    ...
    s.loopToVectorDim[c->m->getMatchedOperation()] = 0;
    vectorizeRootMatch(*(c->m), &s);
   // c->m->getMatchedOperation()->dump();
  }
  LLVM_DEBUG(dbgs() << "\n");
}
...
mlir::createOuterLoopVecPass(llvm::ArrayRef<int64_t> virtualVectorSize) {
  return std::make_unique<OuterLoopVec>(virtualVectorSize);
}

static PassRegistration<OuterLoopVec>
    pass("outer-loop-vectorize",
         "A simple outer loop vectorization pass");
